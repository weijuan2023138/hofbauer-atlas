...
为量化单细胞水平的功能程序，使用Seurat的AddModuleScore函数（ctrl=30）对正常早孕和晚孕Hofbauer细胞计算模块评分。首先在8个精细功能模块层面计算评分：（1）祖细胞与分化模块（Progenitor & Differentiation），包含TREM2、AXL、CEBPA、ID2、CD5L、NOTCH2、SOX4、MAFB、HMGA2；（2）增殖模块（Proliferation），包含MKI67、TOP2A、CCNA2、PCNA、BIRC5、CDK1、AURKB、CDC20、CCNB1；（3）糖酵解模块（Glycolysis），包含ENO1、PGK1、LDHA、HK2、GAPDH、PKM、ALDOA、TPI1；（4）ECM重塑模块（ECM Remodeling），包含FN1、PDGFB、COL1A2、MMP14、TIMP1、VIM、COL4A1、COL6A1、SPARC；（5）氧化应激模块（Oxidative Stress），包含SOD2、PRDX1、CAT；（6）炎症模块（Inflammation），包含CCL8、IL18、NLRP3、TNF、IL1B、NFKB1、RELB、CXCL8、JUNB；（7）抗原呈递模块（Antigen Presentation），包含HLA-DRA、HLA-DRB1、CTSS、IFI30、CD74、HLA-DMA、HLA-DMB、FCER1G；（8）补体模块（Complement），包含C1QA、C1QB、C1QC、C3、C4A、CFB、CFH、SERPING1。为简化展示，将上述8个模块按功能类别合并为5个主模块：祖细胞模块（模块1）、增殖模块（模块2）、代谢模块（模块3+5）、重塑模块（模块4）、免疫模块（模块6+7+8）。模块评分在早孕和晚孕组间使用Wilcoxon检验进行差异比较，显著性标注为***p<0.001、**p<0.01、*p<0.05、ns不显著。

作为发育轨迹的补充分析，使用Slingshot对正常早孕和晚孕Hofbauer细胞的Harmony UMAP嵌入进行拟时间轨迹推断。以Pro-inflammatory亚型为起始cluster（start.clus），使用默认stretch参数（stretch=2）和200个近似点（approx_points=200）拟合主曲线。提取主谱系的拟时间值后，沿拟时间轴使用loess平滑（span=0.3）拟合三个简化功能模块的评分变化曲线：发育模块（TREM2、AXL、CEBPA、ID2、CD5L、NOTCH2）、重塑模块（TIMP1、VIM、MMP14、ENO1、PGK1、COL1A2、FN1、PDGFB、SOD2）和免疫模块（CCL8、IL18、IFI30、HLA-DRA、CTSS、FCGR3A）。
...
De novo motif富集分析针对足月特异性开放peak进行：使用MEME Suite中的AME工具（Analysis of Motif Enrichment）对JASPAR 2022脊椎动物核心motif数据库进行扫描，E-value<1×10⁻⁵视为显著富集。单个peak中motif出现频率的定量使用FIMO（p<1×10⁻⁴），统计每个motif在中期和足月peak中的覆盖率百分比，使用Fisher精确检验比较组间差异。对于图2c的de novo motif富集分析，在足月特异性开放peak（n=373）中使用AME对JASPAR数据库进行扫描（E-value<1×10⁻⁵）。对于图6c的基因组范围motif分布分析，使用FIMO分别扫描STAT3（MA0144.3）、STAT1（MA0137.3）、RELB（MA1117.1）、CEBPA（MA0102.4）和NFKB1（MA0105.4）的结合基序在全部23,574个差异ATAC峰（|log2FC|>0.25，FDR<0.05）中的出现频率。对于图6中通讯基因调控元件的motif扫描，提取30个核心通讯基因转录起始位点±50kb范围内的差异peak序列进行同样的FIMO扫描。
...
Hofbauer细胞亚型间通讯分析使用CellChat对6个亚型间的配体-受体互作进行独立分析。输入数据为Hofbauer_Atlas_Final.rds中按亚型标签（subtype）分组的表达矩阵，每个亚型随机下采样至最多200个细胞（set.seed=42），以保证各亚型细胞数均衡。使用CellChatDB.human数据库，以标准参数运行：truncatedMean计算平均表达（trim=0.1），projectData投射到PPI.human，computeCommunProb使用triMean方法，filterCommunication过滤低概率互作（min.cells=5）。分析结果以亚型间互作数量热图和通路级弦图展示。为比较疾病状态下亚型间通讯的变化，对正常、PE、PTB、流产和感染五组分别运行上述流程，以正常组为基准计算各疾病组的互作数量差异矩阵（Δ interaction count），使用蓝-白-红配色热图可视化。亚型通讯基因的表达模式以DotPlot展示，基因按功能分为四个模块：ECM模块（SPP1、FN1、COL1A1、COL1A2、MMP9、THBS1、ITGAV、ITGB1、ITGA1、ITGA2、ITGB3、ITGB5）、TGF-β模块（TGFB1、BMP2）、生长因子模块（IGF1、VEGFA、HGF、CSF1）和免疫模块（TNF、IL1B、IL6、IL10、CXCL8、CCL2、CCL3、CCL4、CXCL2、CD44、CD47、PTPRM）。
...
TF-通讯基因相关性分析：选取CEBPA、STAT3、STAT1、NFKB1和RELB五个关键转录因子与30个核心通讯基因（SPP1、FN1、COL1A2、COL1A1、TGFB1、IGF1、PTPRM、CD44、ITGAV、ITGB1、ITGA1、ITGA2、ITGA5、ITGA9、ITGB5、SDC1、SDC2、SDC4、CD47、THBS1、LAMC1、COL4A1、COL4A2、VEGFA、VEGFB、BMP2、BMP4、GDF5、MIF、MDK），在所有17,896个Hofbauer细胞中提取两个基因集均有表达的13,582个细胞，计算Spearman秩相关系数，使用Benjamini-Hochberg方法对p值进行FDR校正。相关性矩阵以ComplexHeatmap可视化，按TF（行）和通讯基因（列）排列，色阶为蓝（负相关）-白（零）-红（正相关），显著性以星号标注。
...
Hofbauer细胞特征蛋白的免疫荧光和流式细胞术验证数据来自Arutyunyan等人（2023）的多组学研究和GSE290578数据集。8个Hofbauer特征蛋白（FOLR2、CD163、MRC1、CD68、FCGR3A、HLA-DRA、TREM2、C1QA）的蛋白水平表达与对应基因的转录本表达进行跨孕期比较，以评估转录分层在蛋白水平的一致性。

## 数据可用性

本研究中使用的公开数据集可通过GEO数据库获取：GSE290578、GSE214607、GSE173193、GSE183338、GSE298119。Arutyunyan等人（2023）的多组学数据可通过其论文获取。hoo_2024数据待发表。UCSF_Li_2026（Li等人，2026）的scRNA-seq、snATAC-seq和Stereo-seq数据GEO accession待补充。my_preterm_cohort为自有数据。所有分析代码可在GitHub获取（https://github.com/待补充）。
