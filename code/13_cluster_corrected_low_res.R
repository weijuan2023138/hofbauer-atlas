#!/usr/bin/env Rscript
# Cluster corrected Hofbauer cells with lower resolution (6-8 clusters)
# Input: all_hofbauer_final_corrected.h5ad
...
# Paths
INPUT <- '/home/weijuan/文档/胎盘单细胞数据/ucsf_integration/results/classification/all_hofbauer_final_corrected.h5ad'
OUTPUT_DIR <- '/home/weijuan/文档/胎盘单细胞数据/ucsf_integration/results'

cat("=" , rep("=", 59), "\n", sep="")
cat("Cluster Corrected Hofbauer Cells (6-8 clusters)\n")
cat("=" , rep("=", 59), "\n", sep="")

# 1. Load data
cat("\n[1/7] Loading corrected Hofbauer cells...\n")
...
seu <- CreateSeuratObject(counts = mat, project = 'Hofbauer_Atlas_Corrected')
...
# 5. UMAP + Clustering with LOW resolution (6-8 clusters)
cat("\n[5/7] UMAP + Clustering with LOW resolution...\n")
seu <- RunUMAP(seu, reduction = 'harmony', dims = 1:30, verbose = FALSE)
seu <- FindNeighbors(seu, reduction = 'harmony', dims = 1:30, verbose = FALSE)

# Try very low resolutions
for (res in c(0.1, 0.15, 0.2, 0.25, 0.3)) {
...
# Use resolution 0.15 as default (should give 6-8 clusters)
seu <- FindClusters(seu, resolution = 0.15, verbose = FALSE)

# 6. Save clustered data
cat("\n[6/7] Saving clustered data...\n")
output_file <- file.path(OUTPUT_DIR, 'hofbauer_corrected_clustered.rds')
...
# Save full DEG table
write.csv(am, file.path(OUTPUT_DIR, 'all_markers_corrected.csv'), row.names = FALSE)
cat("  Saved all_markers_corrected.csv\n")

# Output top 10 markers per cluster
cat("\nTop 10 markers per cluster:\n")
cat(rep("=", 80), "\n", sep="")
sink(file.path(OUTPUT_DIR, 'top10_markers_per_cluster_corrected.txt'))
...
sink()
cat("  Saved top10_markers_per_cluster_corrected.txt\n")
...
write.csv(avg_df, file.path(OUTPUT_DIR, 'key_gene_expression_corrected.csv'), row.names = FALSE)
cat("  Saved key_gene_expression_corrected.csv\n")
...
cat("\nOutput files:\n")
cat("  1. hofbauer_corrected_clustered.rds - Corrected Hofbauer Seurat object\n")
cat("  2. top10_markers_per_cluster_corrected.txt - Top 10 DEG per cluster\n")
cat("  3. key_gene_expression_corrected.csv - Key gene expression matrix\n")
cat("  4. all_markers_corrected.csv - Full DEG table\n")

cat("\n", rep("=", 60), "\n", sep="")
cat("Done! Expected 6-8 clusters.\n")
cat(rep("=", 60), "\n", sep="")
