#!/usr/bin/env Rscript
# Cluster all 25,961 Hofbauer cells and export DEG tables
# Input: all_hofbauer_final.h5ad (from Python Harmony)
...
# Paths
INPUT <- '/home/weijuan/文档/胎盘单细胞数据/ucsf_integration/results/classification/all_hofbauer_final.h5ad'
OUTPUT_DIR <- '/home/weijuan/文档/胎盘单细胞数据/ucsf_integration/results'

cat("=" , rep("=", 59), "\n", sep="")
cat("Cluster All Hofbauer Cells + Export DEG Tables\n")
cat("=" , rep("=", 59), "\n", sep="")
...
output_file <- file.path(OUTPUT_DIR, 'all_hofbauer_clustered.rds')
...
cat("  Saved all_markers_final.csv\n")

# Output top 15 markers per cluster
cat("\nTop 15 markers per cluster:\n")
cat(rep("=", 80), "\n", sep="")
sink(file.path(OUTPUT_DIR, 'top_markers_per_cluster_final.txt'))
...
sink()
cat("  Saved top_markers_per_cluster_final.txt\n")
...
# Save
write.csv(avg_df, file.path(OUTPUT_DIR, 'key_gene_expression_final.csv'), row.names = FALSE)
cat("  Saved key_gene_expression_final.csv\n")
...
cat("\nOutput files:\n")
cat("  1. all_hofbauer_clustered.rds - Clustered Seurat object\n")
cat("  2. top_markers_per_cluster_final.txt - DEG tables for annotation\n")
cat("  3. key_gene_expression_final.csv - Key gene expression matrix\n")
cat("  4. all_markers_final.csv - Full DEG table\n")
